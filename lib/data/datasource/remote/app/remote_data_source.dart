    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonResponse['success'] == true) {
          ...
          Get.snackbar(
            "فشل",
            jsonResponse['message'] ?? "فشلت عملية فتح الحساب",
  ...
  Future<TicketModel?> openNewTicket({
    ...
    try {
      ...
      if (response.statusCode == 201) {
        ...
        Get.snackbar(
          "فشل",
          "فشلت عملية فتح التذكرة. رمز الحالة: ${response.statusCode}",
        );
        print(
          "API Error (Open Ticket): ${response.statusCode} - ${response.body}",
        );
        return null;
      }
    } catch (e) {
      ...
      Get.snackbar(
        "خطأ اتصال/خادم",
        "تعذر الاتصال بفتح التذكرة. الرمز: ${e.runtimeType}",
      );
  ...
  Future<bool> addMessageToTicket({
    ...
    try {
      ...
      if (response.statusCode == 201 || response.statusCode == 200) {
        ...
        Get.snackbar(
          "فشل",
          "فشلت عملية إرسال الرسالة. رمز الحالة: ${response.statusCode}",
        );
        print(
          "API Error (Add Message): ${response.statusCode} - ${response.body}",
        );
        return false;
      }
    } catch (e) {
